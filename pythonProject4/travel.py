...
class Admin:
    ...
    def update_extra_team(self, team_id, new_destination, new_start_date, new_end_date, new_transport_mode):
        """Update travel plan details for a specific team (extra functionality)."""
        team = self.teams.get(team_id)
        if team:
            # Assuming we are updating travel plans here
            if team.travel_plans:
                for plan in team.travel_plans:
                ...
                print(f"Travel plans for Team ID {team_id} updated.")
            else:
                print(f"No travel plans exist for Team ID {team_id} to update.")
        else:
            print(f"Team ID {team_id} not found.")
        ...
        while True:
            ...
            if choice == '1':  # Admin Operations
                ...
                if user_id not in self.team_members:
                    ...
                    self.register_team_member(user_id, name)
